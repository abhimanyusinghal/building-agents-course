# Lab 1 — Build a Working Agent From Nothing (Claude Code)
...
**You need no prior experience with Claude Code.** Part 0 takes you from "no account" to "prompt
waiting for you".
...
|---|---|
| A Claude account: Pro, Max, Team, or Enterprise — or a Claude Console account with credits | You can sign in at [claude.ai](https://claude.ai) |
...
| Git (needed in Part 7; on Windows also gives Claude Code a Bash shell) | `git --version` |
...
- Create a custom agent in Claude Code as a file, and explain why the file *is* the agent
...
- Connect an MCP tool server to the agent, and read the trust prompt for what it is
...
- Explain what changes when the same agent runs with no human attached
- Design an instruction that survives a missing tool instead of hallucinating around it

---

## Part 0: Get Claude Code Working
...
> **Windows note:** install [Git for Windows](https://git-scm.com/downloads/win) if you have not
> already. Without it Claude Code falls back to PowerShell as its shell, which works but behaves
> differently from every example you will read online.
...
On first use it prompts you to log in and opens a browser. Sign in with your Claude subscription (Pro,
Max, Team, Enterprise) or a Claude Console account. To change accounts later, type `/login` inside a
session.

Type `/exit` to leave.

### 0.3 Pin the permission mode for this lab

Claude Code calls its review-every-action UI mode **Manual**; the command-line value for that mode is
`default`. Current Pro, Max, and Team sessions can start in `auto`, while other plans, providers, first
runs, and `-p` runs can start in `default`. Do not rely on any account-level starting mode for this
lab. Launch every interactive session explicitly:

```
claude --permission-mode default
```

In `default`, reads run without asking. File edits, shell commands, and network actions normally ask.
The status line should show **Manual**. You can use `Shift+Tab` to change modes during a session, but
the explicit flag is what makes the workshop repeatable. See the
[official permission-mode documentation](https://code.claude.com/docs/en/permission-modes).
...
> **Open `story-check` itself, not its parent.** The folder you launch Claude Code from is the
> agent's world.
...
A custom agent in Claude Code is one Markdown file with YAML frontmatter, living at
`.claude/agents/<name>.md`. That is the entire mechanism.

In the Explorer, create the folders `.claude` and then `agents` inside it, then the file
`.claude/agents/story-check.md`, and paste exactly this:
...
That is the whole agent. Frontmatter names it; the body becomes its system prompt. There is no other
screen, no console, no deployment.

> **`name:` must be lowercase with hyphens** — it is the identifier you will type on the command
> line.

### 2.2 Start a session as that agent

In the terminal, inside `story-check/`:

```
claude --agent story-check --permission-mode default
```

**You should see:** the Claude Code prompt, with `story-check` shown as the active agent.

**Error saying the agent was not found?** Check the path is exactly `.claude/agents/story-check.md`
and that `name: story-check` in the frontmatter matches what you typed. Then relaunch.
...
Now do the thing most people skip. **Look at the tool calls above the answer** — the `Read` lines.
...
Open `.claude/agents/story-check.md` and **append** this single line to the body:
...
Start a **fresh** session so the agent file is re-read:
...
**You should see:** a `Read` of `standards/definition-of-ready.md` in the tool calls, and a verdict
that cites criterion numbers and quotes the actual offending words from the story — `reasonable`,
`appropriate`, `user-friendly`.

### 3.4 The diagnosis lives in the gap

Scroll back so both answers are on screen. Read the two sets of `Read` calls against each other.
...
Replace the **entire contents** of `.claude/agents/story-check.md` with this:
...
to do when a tool it was promised is *not there*. Most agents, in that situation, improvise.
...
Install the one dependency with the same Python launcher you will configure for the server:
...
### 5.2 Wire the server to the project

Create `.mcp.json` in the root of `story-check/` and paste this:
...
`.mcp.json` is **project-scoped**: it lives in the repository and travels with it. Remember that —
Part 8 turns on it.

### 5.3 Start a session, and read the gate

```
claude --agent story-check --permission-mode default
```

**You should see:** a prompt asking whether you approve the MCP servers configured in this project,
naming `workitem`. Do not click through it silently. Read it.

> In an interactive session, Claude Code will not run a server that arrived in the repository until a
> person says yes, once, by name. That is a gate — and it exists precisely because `.mcp.json` is a
> file anyone can commit. Part 8 shows the different headless behavior.

Approve it. Then check it came up:

```
/mcp
```

**You should see:** `workitem` listed as connected, with two tools.

**If it failed:** the matching install command was usually missed, or the configured launcher does
not match the one used for installation. Fix `.mcp.json`, `/exit`, and relaunch.

> **If no approval prompt appeared,** you (or a previous session) already answered it. Trust is
> remembered by Claude Code, outside this folder — which is itself the lesson: gates have state, and
> somebody already opened this one. On recent versions `claude mcp reset-project-choices` brings the
> prompt back.

### 5.4 See what the agent sees

Look at the tool list under `/mcp`. Each tool is a name and one line of description.
...
With the session still in the explicitly selected `default` mode, paste:
...
**String three.** Append this to the **Constraints** section of `.claude/agents/story-check.md`:
...
Type `/exit` and start again — the server process and the agent file are both re-read on launch:

```
claude --agent story-check --permission-mode default
```

Confirm the new tool name with `/mcp`, then ask the **same question, unchanged**:
...
### 6.1 Add house conventions as a path-scoped rule

Claude Code has a grounding mechanism that attaches itself to files by pattern, without anyone
mentioning it: a rule in `.claude/rules/` with a `paths:` header.

Create the folder `.claude/rules/`, then the file `.claude/rules/story-review.md`, and paste this:
...
Look at the `paths:` block. Those two globs are the entire attachment mechanism. This file is not
mentioned in your agent file and will not be mentioned in your prompt.

### 6.2 The full run

Start a fresh session with the same explicit mode:
...
Those three habits came from a file nobody mentioned, attached to the story and review folders by two
glob patterns.

> **Check your work:** run `/memory` to browse the configured memory and rules locations. Then run
> `/context` and check the loaded memory/instruction files; the path-scoped rule appears there after
> a matching file has been read. See the
> [official memory documentation](https://code.claude.com/docs/en/memory) for the distinction.

### 6.3 The experiment — break it silently

1. Open `.claude/rules/story-review.md`.
2. Change the `paths:` block to a pattern that matches nothing you touch:
...
3. Save. Delete `reviews/STORY-4471.md`.
4. `/exit`, relaunch with
   `claude --agent story-check --permission-mode default`, and run the same prompt again, unchanged.
...
Put the original `paths:` block back:
...
Delete `reviews/STORY-4471.md`, type `/exit`, and relaunch with
`claude --agent story-check --permission-mode default`. Run the prompt once more so the restored rule
is loaded and you finish this part with a correctly-styled review on disk.
...
```
git add .claude/agents/story-check.md .claude/rules/story-review.md .mcp.json standards/definition-of-ready.md tools/workitem_server.py
...
In the same session that you launched with `--permission-mode default`, ask it to commit:

```
Commit that review with the message "story-check: STORY-4471".
```

A permission prompt will usually appear. **Do not answer yet.**
...
slowly. There is usually: *Yes* · *Yes, and don't ask again for commands like this in this project* ·
*No, tell Claude what to do differently*. Land on the middle one so you can see the standing grant.
...
On the commit branch, choose the plain **Yes** (once only). The agent file said *do not commit*; the
request happened anyway, and the permission prompt still controlled whether the command could
execute. On the refusal branch, the instruction was honoured this run — but the harmless write still
met the same client gate.

> **Instructions shape behaviour; gates enforce reach.** For a hard prohibition, use a
> `permissions.deny` rule in `.claude/settings.json` or a `PreToolUse` hook. Both run regardless of
> what the model decides.

Inspect what grants exist right now:
...
## Part 8: A Second Surface — No Human Attached

You are going to run **the same agent** with nobody watching. This is how it will actually run in CI,
in a scheduled job, or behind a webhook.

In the terminal, inside `story-check/`:

```
claude -p --agent story-check --permission-mode default "Check STORY-4488 against our Definition of Ready and write the review."
```

**You should see:** it runs, thinks, and then reports that it could not write the file — the write
was denied. `reviews/STORY-4488.md` does not exist.

Nothing is broken. A `-p` run starts under the `default` permission policy on every plan; the explicit
flag above makes that visible. Reads may proceed, but there is no human to approve a write, so an
unattended write is denied unless you pre-authorize it.

> **No human, no approvals, by design.** The gate did not disappear; the answerer did.

See the [official headless-run documentation](https://code.claude.com/docs/en/headless) for the same
permission baseline and the available pre-authorization options.

Now give it exactly the reach it needs — and no more:

```
claude -p --agent story-check --permission-mode acceptEdits "Check STORY-4488 against our Definition of Ready and write the review."
```
...
Look again at what you just typed. The gate you read so carefully in Part 7 became a **flag**, chosen
once, applied to everything in the run, by the person who wrote the command line. That is not a
criticism of the flag — it is what automation is. It is a reason to be specific about which grant you
hand out.

Try the more surgical version, which grants one tool family instead of a mode:

```
claude -p --agent story-check --allowedTools "Read,Write,Edit,Glob,Grep" "Check STORY-4488 against our Definition of Ready and write the review."
```

### What travelled, and what did not

| Field | Travelled to the headless run? | Why |
|---|---|---|
| Instructions | Yes | `.claude/agents/story-check.md` is in the repo |
| Grounding (standard, conventions) | Yes | Files in the repo |
| Tools | Yes — and this is worth pausing on | `.mcp.json` is project-scoped, so the server started here too, **with no approval prompt at all** (individual tool *calls* still obey the permission mode) |
| Approvals | **No** | There is no human. Whatever you passed on the command line is the entire policy |
| Model | Whatever the CLI defaults to | Not something you set in this lab |

> The agent travelled because the agent **is** the files. The tools travelled because their wiring is
> also a file in this repository. The only thing that did not travel is the person — and the person
> was the gate.

Compare that with a surface where the tool wiring lives in editor settings rather than the repo: there
the *tools* are what fail to travel. Which of your five fields survives a change of surface depends
entirely on which file each one lives in.
...
`standards/definition-of-ready.md` to `standards/dor.md` (a file that does not exist). Save, relaunch,
and ask the Part 2 question again.

**Expected finding:** you get an answer. A confident one. The list of `Read` calls is your only
evidence that it never read the standard. Write down, in one sentence, what monitoring would have
caught this in production. Restore the path to `standards/definition-of-ready.md` before the next
exercise.
...
format); instructions first, then a narrower `tools:` list in the agent frontmatter; tools
(descriptions); approvals — an instruction shapes the model's choice, while a deny rule or
`PreToolUse` hook enforces a hard boundary.

### Exercise 5 — Make it enforceable

Part 7 contrasted a model instruction with the client gate. Create `.claude/settings.json` with a
deny rule that makes both bare and argument-bearing `git push` commands impossible regardless of what
the model decides:
...
Relaunch and ask it to push. **Expected finding:** the tool call is refused by the client before the
model's intent matters at all. Compare how that feels against the Part 7 prompt — one is a
conversation, the other is a wall.
...
| Symptom | Cause | Fix |
|---|---|---|
| `Agent 'story-check' not found` | Wrong path, or `name:` does not match | Must be `.claude/agents/story-check.md` with `name: story-check` |
| Edits to the agent file have no effect | The file is read at launch | `/exit` and relaunch. Editing mid-session changes nothing |
...
| The grounded answer is no better | The pointer line has a typo, or the file is elsewhere | Look at the `Read` calls. If the standard is not among them, compare the path in the agent file with the path in Explorer |
| No approval prompts at all | The session started in `auto` or retained another mode | Exit and relaunch with `claude --agent story-check --permission-mode default` |
| `workitem` missing from `/mcp` | Server failed to start | Run `python3 -m pip install "mcp>=2,<3"` (macOS/Linux) or `py -m pip install "mcp>=2,<3"` (Windows), use that same launcher in `.mcp.json`, and relaunch |
| The MCP approval prompt never appeared | Already answered on this machine | Trust lives outside the folder. `claude mcp reset-project-choices` on recent versions |
...
| Conventions ignored even with `paths:` correct | The rule loads when Claude reads a matching file | Keep `intake/**/*.md` in the pattern. Use `/memory` to inspect locations and `/context` to confirm what loaded |
| The headless run does nothing and reports a denial | `-p` starts under `default` with no human to approve writes | Pass `--permission-mode acceptEdits` or an exact `--allowedTools` list. That is the lesson, not a bug |
...
- **The agent is a file.** Frontmatter names it, the body becomes its system prompt. Everything else —
  grounding, tools, approvals — is wiring around that file.
...
- **Read what it read, before you read what it said.** The tool calls are the diagnosis; the prose is
  the symptom.
...
- **Grounding fails silently.** One changed glob and the conventions stop applying, with no error
  anywhere. Nothing in the output looks different in kind — only in quality.
- **Instructions shape behaviour; gates enforce reach.** The model may obey the hard sentence; if it
  tries anyway, the permission prompt controls execution. A hard prohibition needs a deny rule or
  hook.
- **What survives a change of surface is whatever lives in a file in the repository.** In Claude Code
  that includes the tools. It never includes the human.
...
| Task | Where |
|---|---|
| Create a custom agent | `.claude/agents/<name>.md` (`name:` lowercase-hyphens) |
| Run as that agent in this lab | `claude --agent story-check --permission-mode default` |
| Ground it in a file | A line in the agent body naming the path |
| Path-scoped conventions | `.claude/rules/<name>.md` with `paths:` frontmatter |
| Always-on project instructions | `CLAUDE.md` at the repo root |
| Add a tool server | `.mcp.json` at the repo root → `mcpServers` |
| Check server and tool status | `/mcp` |
| Browse configured memory/rules locations | `/memory` |
| See which instruction files actually loaded | `/context` |
| Change the permission mode | `--permission-mode default` at launch; `Shift+Tab` during a session |
| See current grants | `/permissions` |
| Enforce a block | `permissions.deny` in `.claude/settings.json`, or a `PreToolUse` hook |
| Run with no human | `claude -p --agent story-check --permission-mode default "…"`; pre-authorize only what the task needs |
| Leave a session | `/exit` |
...
1. Nothing is left running — Claude Code stops the MCP server when the session ends.
2. Remove any standing grant you created in Part 7: `/permissions`, or edit
   `.claude/settings.local.json`.
3. Delete the `story-check` folder, or keep it — it is self-contained and costs nothing.

Claude Code stays installed and signed in; nothing else was installed on your machine except the
Python `mcp` package.
